...
현재 Phase 0 기반과 health/time, 로그인·회원가입, 내 정보 조회·수정, 길드원 명단·관리, 마스터 설정·가입 코드, 공지·가격표·보스 통제, 손지원 매칭, 아이템 현황 V2, 콘텐츠 참여 그룹, 공성전 참여, 보스 정의·일정·참여, 보스 참여투표 API가 구현되어 있습니다. Flutter의 현재 `/api/...` 경로는 호환 route로 제공하고 신규 계약은 `/api/v1`을 기준으로 확장합니다.
